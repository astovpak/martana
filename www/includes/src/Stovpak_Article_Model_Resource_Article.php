 ...
 */
class Stovpak_Article_Model_Resource_Article extends Mage_Core_Model_Resource_Db_Abstract
{
    ...
    {
        $this->_init('stovpak_article/article', 'entity_id');
    }
    ...
    public function lookupStoreIds($articleId)
    {
        ...
        $select  = $adapter->select()
            ->from($this->getTable('stovpak_article/article_store'), 'store_id')
     ...
     * @param string $field
     * @param mixed $value
     * @param Stovpak_Article_Model_Article $object
    ...
    protected function _getLoadSelect($field, $value, $object)
    {
        ...
        if ($object->getStoreId()) {
            ...
            $select->join(
                array('article_article_store' => $this->getTable('stovpak_article/article_store')),
                $this->getMainTable() . '.entity_id = article_article_store.article_id',
                array()
            )
            ->where('article_article_store.store_id IN (?)', $storeIds)
            ->order('article_article_store.store_id DESC')
            ->limit(1);
        }
        return $select;
    }
    ...
    protected function _afterSave(Mage_Core_Model_Abstract $object)
    {
        ...
        $table  = $this->getTable('stovpak_article/article_store');
    ...
    protected function _initCheckUrlKeySelect($urlKey, $store)
    {
        $select = $this->_getReadAdapter()->select()
            ...
            ->join(
                array('es' => $this->getTable('stovpak_article/article_store')),
